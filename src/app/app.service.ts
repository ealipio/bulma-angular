...
export class DataService {
  ...
  getCustomerById(id: number): any {
    return JSON.parse(localStorage.getItem('customers')).filter(
  ...
  getOrderById(id: number): any {
    return JSON.parse(localStorage.getItem('orders')).filter(
      orders => orders.id === id
    );
  }
  ...
  getStatistics(when: string): any {
    return JSON.parse(localStorage.getItem('statistics')).filter(
      data => data.when === when
    );
  }
}
